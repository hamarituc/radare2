...
// XXX this is a waste of cycles, it's wrong and awful and needs to be rewritten
static char *is_type(char *type) {
	const char *ktypes[] = {
		"type", "struct", "union", "enum", "typedef", "func", NULL
	};
	int i;
...
static void save_struct(const RAnal *anal, const RAnalBaseType *type) {
	...
	*/
	...
	r_vector_foreach (&type->struct_data.members, member) {
		...
		r_strf_var (k, 128, "%s.%s.%s", kind, sname, member_sname);
		r_strf_var (v, 128, "%s,%u,0", member->type, (unsigned int)member->offset);
...
static void save_union(const RAnal *anal, const RAnalBaseType *type) {
	...
	*/
	...
	r_vector_foreach (&type->union_data.members, member) {
		...
		char *member_sname = r_str_sanitize_sdb_key (member->name);
		r_strf_var (k, 128, "%s.%s.%s", kind, sname, member_sname);
		r_strf_var (v, 128, "%s,%u,%d", member->type, (unsigned int)member->offset, 0);
		sdb_set (anal->sdb_types, k, v, 0);
		free (member_sname);
...
static void save_enum(const RAnal *anal, const RAnalBaseType *type) {
	...
	*/
	...
	r_vector_foreach (&type->enum_data.cases, cas) {
		...
		r_strf_var (param_key, 128, "enum.%s.%s", sname, case_sname);
		r_strf_var (param_val, 128, "0x%" PFMT32x "", cas->val);
		r_strf_var (param_key2, 128, "enum.%s.0x%" PFMT32x "", sname, cas->val);
